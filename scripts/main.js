...
// // Function to check if someone wins or no one
// let gameBoxFields = ["-", "-", "-", "-", "-", "-", "-", "-", "-"];
// let clickedButtons = new Set();
// let gameOver = false;  // Flag to control the game state
...
//     if (wins) {
//         changePara(`The Player ${winner} is the Winner :)`);
//     } else if (clickedButtons.size === gameBox.length) {
//         changePara(`No one wins :|`);
//     }
...
// // Loop for applying listeners and actions after each click
// let playerTurn = "x";

// for (let i = 0; i < gameBox.length; i++) {
//     function functionName() {
//         if (gameOver) return;  // If the game is over, prevent further clicks
...
//                     element.removeEventListener("click", functionName);  // Remove all event listeners
//                 });
//             } else if (clickedButtons.size === gameBox.length) {
//                 changePara(`No one wins :|`);
//                 gameOver = true;  // Set game over flag
